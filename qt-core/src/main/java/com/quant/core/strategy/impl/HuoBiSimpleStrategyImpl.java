...
public class HuoBiSimpleStrategyImpl extends AbstractStrategy implements TradingStrategy, StrategyDelegate {
            ...
            try {
                ...
                if (orderState.getType() == OrderType.BUY) {
                    ...
                    if (this.weights.getSellTotal() != 0 && this.weights.getSellTotal() >= this.baseInfo.getSellAllWeights()
                            && orderState.getType() == OrderType.BUY) {
                        try {
                            createSellOrder();
                        } catch (Exception e) {
                            e.printStackTrace();
                    ...
                    if (this.weights.getBuyTotal() != 0 && this.weights.getBuyTotal() >= this.baseInfo.getBuyAllWeights()
                            && orderState.getType() == OrderType.SELL) {
                        try {
                            createBuyOrder();
                        } catch (Exception e) {
                            redisMqService.sendMsg("当前下单信息【" + this.orderState.toString() + "】==下单失败 重新下单！");
    ...
    public void CalculateProfit() {
        try {
            if (this.orderState.getType() == OrderType.SELL) {
                ...
                final ProfitMessage profitMessage = getProfitMessage(buyOrderId, sellOrderId, diff, divide, ordersBuyDetail, ordersSellDetail, profit);
    ...
    private ProfitMessage getProfitMessage(long buyOrderId, long sellOrderId, BigDecimal diff, BigDecimal divide, OrdersDetail ordersBuyDetail, OrdersDetail ordersSellDetail, Profit profit) {
